...
awful.layout.layouts = {
    awful.layout.suit.tile.left, awful.layout.suit.fair
...
awful.screen.connect_for_each_screen(function(s)
    ...
    awful.tag({"terminal", "browser", "code", "chat", "5", "6", "7", "8", "9"},
    ...
    s.mywibox:setup{
        ...
        { -- Right widgets
            wibox.widget.textbox('ţ๏ʍ  '),
            layout = wibox.layout.fixed.horizontal,
            brightness_widget {timeout = 1},
...
end, {description = "go back", group = "client"}), -- Brightness
...
awful.key({}, "XF86AudioMute", function()
    ...
    awful.util.spawn("pactl set-sink-volume @DEFAULT_SINK@ +3000")
end), awful.key({}, "XF86AudioLowerVolume", function()
    awful.util.spawn("pactl set-sink-volume @DEFAULT_SINK@ -3000")
end), -- Webcam
awful.key({modkey, "Control"}, "w",
          function() awful.util.spawn("webcam show") end),
                              awful.key({modkey, "Control"}, "b", function()
    awful.util.spawn("webcam bumper")
end), awful.key({modkey, "Control"}, "=",
                function() awful.util.spawn("webcam zoom_in") end),
                              awful.key({modkey, "Control"}, "-", function()
    awful.util.spawn("webcam zoom_out")
end), -- Soundboard
awful.key({modkey, "Control"}, "n",
          function() awful.util.spawn("sound-board.bash never-forget") end),
                              awful.key({modkey, "Control"}, "n", function()
    awful.util.spawn("notify-send 'NEVER FORGET'")
end), -- Other
awful.key({modkey}, "i", function() awful.util.spawn("flameshot gui") end),
                              awful.key({modkey}, "b", function()
    local myscreen = awful.screen.focused()
    myscreen.mywibox.visible = not myscreen.mywibox.visible
end, {description = "toggle statusbar"}), -- Standard program
...
awful.rules.rules = {
    ...
    {rule = {class = "Slack"}, properties = {screen = 1, tag = "chat"}}, {
